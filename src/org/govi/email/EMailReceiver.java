...
//				Session session = Session.getDefaultInstance(props, null);
				...
				store.connect("imap.gmail.com", "govind.avireddi", "MyBotKaAddress");
...
// CONTENT EXTRACTION:
		...
		return postMessageToNOSQL(messagedata);
	}
		...
		for(int i = 0; i < multipart.getCount(); i++){
			...
			if(bodypart.getContentType().startsWith("multipart/ALTERNATIVE") == true){
				...
				if (disposition != null && (disposition.equalsIgnoreCase(Part.ATTACHMENT))) {
					String strID = postDocumentToNOSQL(bodypart, "EMAIL_ATTACHMENT");
...
//					DataHandler dh = null;
	 ...
	 */
	private String postDocumentToNOSQL(BodyPart bodypart, String partType) {
		...
		try {
			jsonMetaData = Json.createObjectBuilder()
					.add("contentType", bodypart.getContentType())
					.add("fileName", bodypart.getFileName())
					.build();
...
//	}
	...
	/**
	 * Method: postMessageToNOSQL
	 * Description: Implemented for MongoDB
	 * @param messagedata
	 * @return
	 */
	private int postMessageToNOSQL(MessageData messagedata) {
	...
	public static void main(String[] args) {
		...
		ArrayList<String> arrHits = esclient.searchIndex("stagedemails", null);
